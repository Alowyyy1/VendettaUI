...
local function EnsurePackLoaded(packName)
	if not IconModule.Icons[packName] and PackUrls[packName] then
		local success, pack = pcall(function()
			local content = Get(PackUrls[packName])
...
function IconModule.Icon(Icon, Type, DefaultFormat)
	...
	EnsurePackLoaded(targetType)
	local iconSet = IconModule.Icons[targetType]

	-- Try fallback to sfsymbols or lucide if not found
	if not iconSet or (iconSet.Icons and not iconSet.Icons[targetName] and not iconSet[targetName]) then
		if targetType ~= "sfsymbols" then
			EnsurePackLoaded("sfsymbols")
			if IconModule.Icons.sfsymbols and (IconModule.Icons.sfsymbols[targetName] or (IconModule.Icons.sfsymbols.Icons and IconModule.Icons.sfsymbols.Icons[targetName])) then
				targetType = "sfsymbols"
				iconSet = IconModule.Icons.sfsymbols
			end
		end
		if not iconSet and targetType ~= "lucide" then
			EnsurePackLoaded("lucide")
			if IconModule.Icons.lucide and (IconModule.Icons.lucide[targetName] or (IconModule.Icons.lucide.Icons and IconModule.Icons.lucide.Icons[targetName])) then
				targetType = "lucide"
				iconSet = IconModule.Icons.lucide
			end
		end
	end
	...
	elseif iconSet and iconSet[targetName] and string.find(tostring(iconSet[targetName]), "rbxassetid://") then
